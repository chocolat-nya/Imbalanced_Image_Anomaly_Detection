...
from .params import get_model_params, NOISY_STUDENT_WEIGHTS
...
def _get_model_by_name(model_name, input_shape=None, include_top=True, weights=None, classes=1000, pooling=None):
    ...
    if weights not in {None, 'noisy-student'}:
        raise ValueError('Parameter `weights` should be one of [None, "noisy-student"]')

    if weights == 'noisy-student' and model_name not in NOISY_STUDENT_WEIGHTS:
        raise ValueError('There are not pretrained weights for {} model.'.format(model_name))

    if weights == 'noisy-student' and include_top and classes != 1000:
    ...
    if weights:
        ...
        weights = NOISY_STUDENT_WEIGHTS[weights_name]
